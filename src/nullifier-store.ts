...
/**
 * Extract nullifiers from Transact events and persist them to chain storage.
...
async function syncNullifiers (
  ...
  await chainStorage.insertNullifiersBatch(batch)
...
async function loadNullifierSet (
  ...
  if (cache) {
    if (!cache.isInitialized) {
      await cache.initialize(chainStorage)
    } else {
      await cache.update(chainStorage)
    }
