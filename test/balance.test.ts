...
test('single token all unspent', (t) => {
  ...
  if (balance) {
    t.is(balance.token, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 'correct token')
    t.is(balance.balance, 600n, 'correct balance sum')
    t.is(balance.utxos.length, 3, 'correct utxo count')
    t.alike(balance.utxos, notes, 'utxos array contains all notes')
  }
})
...
test('single token some spent', (t) => {
  ...
  const balance = result[0]
  if (balance) {
    t.is(balance.balance, 400n, 'correct balance sum (100 + 300)')
    t.is(balance.utxos.length, 2, 'correct utxo count')
    t.is(balance.utxos[0]?.nullifier, '0x01', 'first unspent note included')
    t.is(balance.utxos[1]?.nullifier, '0x03', 'second unspent note included')
  }
})
...
test('zero-value notes handled correctly', (t) => {
  ...
  if (result[0]) {
    ...
    t.is(result[0].utxos.length, 4, 'all notes counted including zero-value')
  }
})
