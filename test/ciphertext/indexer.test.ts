...
test('CiphertextIndexer: array ingestion, primary/secondary index', async () => {
  ...
  assert.ok(indexer.exists('20750167656587297163013996512708175612769165796089350936909661845093401229266'))
  assert.ok(indexer.exists('4334396287621752140794985571520419684742564655467396393269301406520235655169'))
  assert.equal(indexer.exists('12'), false)
  ...
  assert.equal(rec1?.transactionHash, '0xa2b5912c3341ad0cae31552fa77fb94fbb03a9deb846a5a4742e8bd90a500492')

  const rec2 = indexer.get('4334396287621752140794985571520419684742564655467396393269301406520235655169')
  assert.equal(rec2?.transactionHash, '0xdc3f15f5caa3b303d74f8139543b4132b51e3dd2a2452b461f1ee81e711f684f')
  ...
  assert.equal(bySender[0]?.transactionHash, '0xa2b5912c3341ad0cae31552fa77fb94fbb03a9deb846a5a4742e8bd90a500492')
  ...
  assert.equal(byReceiver.length, 1)
  assert.equal(byReceiver[0]?.transactionHash, '0xa2b5912c3341ad0cae31552fa77fb94fbb03a9deb846a5a4742e8bd90a500492')
  ...
  assert.equal(byNonExistentIndex.length, 0)
  ...
  assert.deepEqual(indexer.getAllPrimaryKeys().sort(), allPKs.sort())
  ...
  assert.deepEqual(decoded, { decoded: true, key: '0xviewkey', hash: '0xdc3f15f5caa3b303d74f8139543b4132b51e3dd2a2452b461f1ee81e711f684f' })
})

test('CiphertextIndexer: async iterable ingestion', async () => {
...
test('CiphertextIndexer: composite primary key', async () => {
  /**
